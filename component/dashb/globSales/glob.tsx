import styles from "./glob.module.css";
import LanguageIcon from "@mui/icons-material/Language";
import SvgIcon from "@mui/icons-material/Language";
import Map from "./map";
import { useEffect, useState } from "react";
import { FormControl, InputLabel, MenuItem, Select } from "@mui/material";

const Glob = () => {
  const [scale, setScale] = useState<number>(1);
  const [regions, setRegions] = useState<Array<Object>>([]);
	useEffect(() => {
	  setRegions(getData());
	}, [])
	const getData = () => {
		let myimg1 = 'data:image/png;base64,';
		let myimg2 = 'data:image/png;base64,';
		let myimg3 = 'data:image/png;base64,';
		let myimg4 = 'data:image/png;base64,';
		let myimg5 = 'data:image/png;base64,';
	
		return [
		{
			region: {
				name: 'USA',
				data: '2.920',
				dataPercent: '53.23%',
				img: myimg1
			}
		},
		{
			region: {
				name: 'Germany',
				data: '1.300',
				dataPercent: '20.43%',
				img: myimg2
			}
		},
		{
			region: {
				name: 'Australia',
				data: '760',
				dataPercent: '10.35%%',
				img: myimg3,

			}
		},
		{
			region: {
				name: 'United Kingdom',
				data: '690',
				dataPercent: '7.87%',
				img: myimg4,

			}
		},
		{
			region: {
				name: 'Romania',
				data: '600',
				dataPercent: '5.94%',
				img: myimg5,

			}
		},
	]
  }
  return (
   
    <div className={styles.glob}>
      <div>
        <h3>Regions Select by</h3>

	  <FormControl sx={{ m: 1, minWidth: 80 }}>
        <InputLabel id="demo-simple-select-autowidth-label">name</InputLabel>
        <Select
          labelId="demo-simple-select-autowidth-label"
          id="demo-simple-select-autowidth"
          // value={age}
          onChange={getData}
          autoWidth
          label="Age"
        >
          <MenuItem value="">
            <em>None</em>
          </MenuItem>
          <MenuItem value={1}>Population</MenuItem>
          <MenuItem value={2}>Covide-19</MenuItem>
          <MenuItem value={3}>Vastness</MenuItem>
          <MenuItem value={4}>Climate</MenuItem>
          <MenuItem value={5}>Meknès-Tafilalet </MenuItem>
          <MenuItem value={6}>Rabat-Salé-Zemmour-Zaer</MenuItem>
          <MenuItem value={7}>Grand Casablanca</MenuItem>
          <MenuItem value={8}>Chaouia-Ouardigha </MenuItem>
          <MenuItem value={9}>Doukhala-Abda</MenuItem>
          <MenuItem value={10}>Marrakech-Tensift-Al Haouz</MenuItem>
          <MenuItem value={11}>Tadla-Azilal </MenuItem>
          <MenuItem value={12}>Souss-Massa-Drâa</MenuItem>

        </Select>
      </FormControl>
      </div>
      <div className={styles.logo}>
        <SvgIcon
          component={LanguageIcon}
          inheritViewBox
          className={styles.globLogo}
        />
      </div>
	  			
    <div className={styles.countryStatic}>
        <div className={styles.statistics}>
			<table className={styles.tb}>
				<tbody>
					{
						regions?.map((elemnt: Object, indx) => {
							
						return (
								<tr key={indx}>
									<td>
									<img
										src={elemnt?.region.img}
										alt="us_flag"
									/>
									</td>
									<td>{elemnt?.region.name}</td>
									<td>{elemnt?.region.data}</td>
									<td>{elemnt?.region.dataPercent}</td>
								</tr>
						)
					})
					}
				</tbody>
			</table>
        </div>
        <div className={styles.map}>
          <Map scale={scale}/>
          <div
            className={styles.zoomIn}
            onClick={() => {
              setScale((curr) => curr + 0.5);
            }}
          >
            +
          </div>
          <div
            className={styles.zoomOut}
            onClick={() => {
              setScale((curr) => {
                if (curr > 1) return curr - 0.3;
                else return curr;
              });
            }}
          >
            -
          </div>
        </div>
      </div>
    </div>
  );
};

export default Glob;
